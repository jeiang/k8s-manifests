...
- The Hagezi denylist URLs are pinned to a specific jsDelivr tag (not `@latest`) so an upstream push can't change cluster-wide DNS behavior without a reviewed values change; bump the pinned tag deliberately (check https://github.com/hagezi/dns-blocklists/tags). The StevenBlack URLs remain on `master` because StevenBlack's tagged releases don't consistently include the `alternates/gambling-only/hosts` variant this chart uses (confirmed 404 across the 3 most recent tags).
